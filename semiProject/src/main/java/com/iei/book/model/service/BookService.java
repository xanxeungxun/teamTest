...
public class BookService {
	...
	public BookListData selectGenreBook(int selectGenreCode, int reqPage) {
		...
		//페이징 제작
		//필요한 전체 페이지수 구하기 <-- 일단 전체 작품 수 알아야함
		int totalCount = dao.selectGenreBookCount(conn); //전체 작품수
		int totalPage = (int) Math.ceil((double)totalCount/numPerPage); //네비게이터 전체 페이지수
		int naviSize = 5;
		int naviStart = ( ((reqPage-1)/naviSize) * naviSize )+1;
		//↑내가 5번페이지가 보고싶으면, navi시작번호는 1번임, 6페이지부터 5임
	...
	public BookListData selectAllBookComplete(int reqPage) {
		Connection conn = JDBCTemplate.getConnection();
		...
		int end = numPerPage * reqPage; // 보여줄작품개수*요청페이지
		int start = (end - numPerPage) + 1;
		ArrayList<Book> bookList = dao.seletAllBookComplete(conn,start,end);
		
		//페이징 제작
		//필요한 전체 페이지수 구하기 <-- 일단 전체 작품 수 알아야함
		int totalCount = dao.selectBookCountComplete(conn); //전체 작품수
		int totalPage = (int) Math.ceil((double)totalCount/numPerPage); //네비게이터 전체 페이지수
		int naviSize = 5;
		int naviStart = ( ((reqPage-1)/naviSize) * naviSize )+1;
		//↑내가 5번페이지가 보고싶으면, navi시작번호는 1번임, 6페이지부터 5임
	...
	public BookListData selectAllBookViews(int reqPage) {
		Connection conn = JDBCTemplate.getConnection();
		...
		int end = numPerPage * reqPage; // 보여줄작품개수*요청페이지
		int start = (end - numPerPage) + 1;
		ArrayList<Book> bookList = dao.seletAllBookViews(conn,start,end);
