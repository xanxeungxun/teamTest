...
public class BookListManageDao {
	...
	public ArrayList<BookListManageVo> selectAllBookListManageList(Connection conn,int start,int end) {
		...
		String query = "select * from(select rownum as rnum, n.* from(select book_no,genre_code,book_title,book_writer,book_status,book_date from book order by 1 desc)n)where rnum between ? and ?";
		try {
			...
			while(rset.next()) {
				...
				n.setGenreCode(rset.getInt("genrecode"));
