...
public class UserManageService {
	...
	public UserManagePageData selectSearchedUserManage(int reqPage, UserManagePageData paramVo) {
		Connection conn = JDBCTemplate.getConnection();
		...
		ArrayList<UserManageVo> list = dao.selectSearchedUserManage(conn,start,end, searchValue, searchType);
		int totalCount = dao.selectUserManageCount(conn);
		int totalPage = 0;
		if(totalCount%numPerPage == 0) {
			totalPage = totalCount/numPerPage;
		...
		if(pageNo != 1) {
			pageNavi += "<li>";
			pageNavi += "<a class='page-item' href='/userManageList.do?reqPage="+(pageNo-1)+"'>";
			pageNavi += "<span class='material-icons'>chevron_left</span>";
			pageNavi += "</a></li>";
		}
		for(int i=0;i<pageNaviSize;i++) {
